/* ================================================================
   OCHA Map — D3 renderer — Burkina Faso (17 régions ADM1)
   GeoJSON embedded in burkina-data.js → no fetch, no CORS issues
   ================================================================ */
...
function renderOchaMap({
  ...
  colorRange,
  labelFormat,
  colorInterp   // [lightColor, darkColor], defaults to UN Blue ramp
}) {
  const svgEl = document.getElementById(svgId);
  if (!svgEl) { console.error('SVG #' + svgId + ' not found'); return; }
  ...
  /* ── use embedded GeoJSON (no fetch) ── */
  if (typeof BURKINA_GEOJSON === 'undefined') {
    svg.append('text')
      .attr('x', W / 2).attr('y', H / 2)
      .attr('text-anchor', 'middle').attr('font-size', '13').attr('fill', '#6E6259')
      .text('Erreur : burkina-data.js non chargé');
    return;
  }

  const geo = BURKINA_GEOJSON;
  ...
  g.selectAll('.map-region')
    ...
    .attr('class', d => {
      ...
      return 'map-region' + (sid && dataByRegion[sid] ? '' : ' not-surveyed');
    })
    .attr('d', path)
    .attr('fill', d => {
      const sid = d.properties.survey_id;
      ...
      return v != null ? colScale(v) : '#DDDAD7';
    })
    .on('mousemove', function(ev, d) {
      ...
      if (tx + 240 > r.width) tx = ev.clientX - r.left - 240;
  ...
  /* ── north arrow ── */
  svg.append('text').attr('x', W - 18).attr('y', 26)
    .attr('text-anchor', 'middle').attr('font-size', '15')
    .attr('fill', '#AEA29A').attr('pointer-events', 'none').text('N');
  svg.append('text').attr('x', W - 18).attr('y', 14)
    .attr('text-anchor', 'middle').attr('font-size', '14')
    .attr('fill', '#AEA29A').attr('pointer-events', 'none').text('↑');
}
